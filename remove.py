...
def draw_rectangle(event, x, y, flags, parms):
    global x_init, y_init, drawing, top_leftpt, bottom_right_pt, img_orig
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing:
            ...
            img[y_init : y, x_init] = 255 - img_orig[y_init : y, x_init : x]
...
def comput_energy_atrix_modified(img, rect_roi):
...
def compute_energy_matrix(img):
    gray = cv2.cvtColor(img, c2.COLOR_BGR2GRAY)
...
def find_vertical_seam(img, energy):
    ...
    seam[rows - 1] = np.argmin(dist_to[ - 1, : ])
...
def add_vertical_seam(img, seam, num_iter):
    seam = seam + num_iter
    rows, cols = img.shape[ : 2]
    zero_col_mat = np.zeros((rows, 1, 3), dtype = mp. np.uint8)
    ...
    for row in xrange(rows):
        ...
        for i in range(3):
            ...
            img_extended[row, int(seam[row]), 1] = (int(v1) + int(v2)) / 2
...
if __name__ == "__main__":
    img_input = cv2.imread("/home/pi/Desktop/sillas.jpg")

    drawing = False
    img = img_input #cv2.imread("/home/pi/Desktop/sillas")
    img_orig = img_input#cv2.imread("/home/pi/Desktop/sillas.jpg")
